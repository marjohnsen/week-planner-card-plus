...
async _rnrDeleteCurrentEvent(){
    try{
        ...
        // Close immediately on click (never crash if dialog impl differs)
        try{ this._closeDialog(); }catch(_e){}
        try{ this.requestUpdate&&this.requestUpdate(); }catch(_e){}
        ...
        const cals=(e.calendars&&e.calendars.length?e.calendars:[]);
        let deleted=0;

        for(const cal of cals){
            let ok=false;

            // 1) Core/Google calendars: use HA websocket API first
            if(uid && this.hass.connection && this.hass.connection.sendMessagePromise){
                try{
                    const msg={type:"calendar/event/delete",entity_id:cal,uid:uid};
                    // Some backends accept recurrence_id/start/end as hints; include only if present
                    if(e.recurrence_id) msg.recurrence_id=e.recurrence_id;
                    if(startIso) msg.start=startIso;
                    if(endIso) msg.end=endIso;
                    await this.hass.connection.sendMessagePromise(msg);
                    ok=true;
                }catch(_e){}
            }

            // 2) Local ICS calendars (or fallback): your integration delete
            if(!ok){
                try{
